...
class _DistributedOptimizer(torch.optim.Optimizer):
    ...
    def _generate_merged_parameters(self):
        ...
        groups, key_groupidx_maps = self._generate_groups_with_threshold(self._tensor_fusion_threshold)
        new_keys = []
    ...
    def _push_pull_grad_async(self, d_p, name):
    ...
    def _make_hook(self, p):
        def hook(*ignore):
            ...
            d_p = p.grad
            if d_p in self._handles and self._handles[p.grad][0] is not None:
            ...
            if self._push_pull_delay[p] == 0:
                ...
                #d_p = p.grad
                if self._tensor_fusion_threshold > 0:
                    name, d_p = self._push_to_buffer(name, d_p)
                    if d_p is not None:
                        handle, ctx = self._push_pull_grad_async(d_p, name)
                else:
                    handle, ctx = self._push_pull_grad_async(d_p, name)
            self._handles[d_p] = (handle, ctx)
        return hook
